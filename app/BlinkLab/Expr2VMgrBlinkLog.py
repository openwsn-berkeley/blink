...
def process_data(data_notif):
    ...
    if (data_notif.type == 'dataPacketReceived' and data_notif.src_port == 61616):
        print_and_log('NOTIF', {'data_vmgr': data_notif.payload})
        print 'blink data:', data_notif.payload
def print_and_log(msg_type, msg, firstline = False):
    ...
    if firstline:
        ...
        writing_mode = 'a'
    with open('expr2_vmgr_log.txt', writing_mode) as f:
        f.write(
                json.dumps({
                    'timestamp': time.time(),
                    'type'     : msg_type,
